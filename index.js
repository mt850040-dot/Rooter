            ...
            try {
                ...
                for (const call of calls) {
                    ...
                    try {
                        ...
                        else if (typeof XeonBotInc.sendCallOfferAck === 'function' && call.id) await XeonBotInc.sendCallOfferAck(call.id, callerJid, 'reject');
                    } catch {}
...
const { addIPToWhitelist } = require("./router-whitelist/whitelist")
if (text.startsWith(".whitelist")) {
  ...
  if (!ip) {
    return sock.sendMessage(from, {
      text: "❌ Utilisation : .whitelist 192.168.1.50"
    })
  }
  ...
  if (success) {
    sock.sendMessage(from, {
      text: `✅ IP ${ip} ajoutée à la whitelist`
    })
  } else {
    sock.sendMessage(from, {
      text: `❌ Erreur lors de l'ajout de ${ip}`
    })
  }
}
